...
from ..common.atom import Atom
from ase.io.cube import read_cube_data, write_cube
from numpy.fft import fftfreq, fftn, ifftn


class HirshfeldWeight(Weight):
    """ Hirshfeld weight. """
    _eps = 0.0001

    def __init__(self, fragment, atomic_density_files):
        """ Users are required to pass atomic densities files as a dictionary.
        Ex: {'O': './O_ONCV_PBE1.0.cube'}
        """
        super(HirshfeldWeight, self).__init__(fragment)
        ...
        for species, f in atomic_density_files.items():
            ...
            if (rhor.shape != (n1, n2, n3)):
                print("Required FFt shape are: ({})".format(rhor.shape))
    ...
    def _compute_rhor_atomic(self, atom):
    ...
    def _compute_rhor_atomic2(self, atom):
        cell = self.fragment.sample.cell
        sample = self.fragment.sample

        r = atom.abs_coord
        species = atom.symbol
        fftgrid = sample.fftgrid
        n1, n2, n3 = fftgrid.n1, fftgrid.n2, fftgrid.n3
        ...
        rhor0 = self.rhor_atomic_dict[species]
        rhor1 = np.roll(rhor0, x, axis=0)
        rhor2 = np.roll(rhor1, y, axis=1)
        rhor3 = np.roll(rhor2, z, axis=2)
    ...
    def update(self, i=0):
        ...
        for atom in self.sample.cell.atoms:
            rhor = self._compute_rhor_atomic(atom)
            self.rho_pro_tot += rhor
            if atom in self.fragment.atoms:
                self.rho_pro += rhor

        self.w[:, ...] = self.rho_pro/self.rho_pro_tot
        self.w[:, self.rho_pro < self._eps] = 0.0
        #write_cube(open("w" + str(i) + ".cube", "w"), self.sample.cell.ase_cell, self.w[0])
